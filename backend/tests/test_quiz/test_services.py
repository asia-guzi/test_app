...
@pytest.mark.asyncio
async def test_get_question_in_right_order_success(
    data_for_tests,
    mock_test_service_instance_bounded,
    mock_current_user,
    default_test_size,
):
...
@pytest.mark.asyncio
async def test_get_question_fail(
    data_for_tests,
    mock_test_service_instance_bounded,
    mock_current_user,
    default_test_size,
):
    def compare_redirect_responses(
        response1: RedirectResponse, response2: RedirectResponse
    ) -> bool:
        return response1.status_code == response2.status_code and response1.headers.get(
            "location"
...
@pytest.mark.asyncio
async def test_create_test_with_user(
    mock_async_session, mock_current_user, default_test_size, mock_random_questions
):
    ...
    # call create_test
    test_service_instance = await TestService.create_test(
        user=user, session=session_mock
    )
...
@pytest.mark.parametrize(
    ...
    [
        (0, 0),
        (len(QUESTION_DATA), 100),
        (
            len(QUESTION_DATA) // 2,
            round(len(QUESTION_DATA) // 2 * 100 / len(QUESTION_DATA), 2),
        ),
    ],
)
@pytest.mark.asyncio
...
def test_validate_question_fail(
    mock_user_true_false,
    mock_test_service_instance_bounded,
    mock_current_user,
    data_for_tests,
    mock_validate_answer,
...
def test_validate_answer_cases(
    question,
    collection,
    mock_test_service_instance_bounded,
    mock_current_user,
    ...
    test.validate_answer(
        test_question=get_AnsweredQuestion_schema(question),
        user_answer=collection["true_response"],
    )
    ...
    test.validate_answer(
        test_question=get_AnsweredQuestion_schema(question),
        user_answer=collection["false_response"],
    )
    check_after_score = test.true_ans
    ...
    test.validate_answer(
        test_question=get_AnsweredQuestion_schema(question),
        user_answer=collection["nonexistent_response"],
    )
    check_after_score = test.true_ans
    assert check_before_score == check_after_score
...
@pytest.mark.asyncio
async def test_random_questions_success(
    test_size, mock_async_session, default_test_size
):
    with patch(
        "app.quiz.services.TEST_SIZE", test_size
    ):  # mock global test size vatiable

        result = await TestService.random_questions(
            session=mock_async_session, test_size=test_size
        )
